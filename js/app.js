...
onAuthStateChanged(auth, async (user) => {
  ...
  globalSearch = initGlobalSearch({
    ...
    onOpenProfile:(u)=>{ if(u?.uid===currentUser.uid) openSettingsPanel("#profileSection"); else if(u?.username) location.href=`/u/${encodeURIComponent(u.username)}`; },
...
window.CUNNACTApp={
  syncOwnProfile(partial={}){
    currentUserData={...currentUserData,...partial};
    hydrateCurrentUserUI();
  },
  getCurrentUser:()=>currentUser,
...
/* Static controls */
window.CUNNACTSettingsLogout=()=>logout();
...
function bindStaticControls() {
  ...
  $id("appLockMenuBtn")?.addEventListener("click", async () => {
    ...
    if (isAppLockEnabled(currentUserSettings)) { appLockUnlocked=false; showAppLock(); }
    else { openSettingsPanel("#securitySection"); showToast("App lock is not enabled yet. Open Security to enable it.","info"); }
  });
  ...
  $id("drawerNotificationsBtn")?.addEventListener("click", () => { closeProfileDrawer(); openSettingsPanel("#notificationsSection"); });
